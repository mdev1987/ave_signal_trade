...
class DBotXClient:
    ...
    # NOTE (2026-09-12 review): removed unverified helpers (hot/new/meme/
    # holders/dev) — their REST paths were guessed from doc-page slugs, never
    # confirmed against the DBotX OpenAPI, and nothing in main.py calls them.
    # They silently returned [] (fail-open) so looked integrated while dead.
    # Re-add only with a captured 200 response. pair_safety() below is the
    # single verified endpoint and the only production path.
